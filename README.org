...
=PDFLaTeX.= However, the preamble DOES compile with =PDFLaTeX=, in case you need
quick compilation for math previews etc. Notable things about =XeLaTeX=:
+ =hyperref= works (babel in =PDFLaTeX= screws up compilation with =hyperref=)
